            ...
            while True: #以CV字为单位的子循环
                ...
                if count == 0: #句首时
                    ...
                    if CV_head and not UsePlanB:
                        cv_now = self.findcv_v(headcv_remained,vc_wanted.v) #要求句首CV时，首先尝试写入一个以VC部的V结尾，且还未在句首出现过的CV字
                        if cv_now != None:
                ...
                #检查搜索到的CV字是否符合要求
                vc_wanted_next = self.findcv_v(vc_remained,cv_now.v) #尝试找到下一个需要的VC部，使得其V与目前检索到的CV字的V相同（即能够接续）
                    ...
                    if OtoMaxOfSame == -1 or exist_list.count(_name) < OtoMaxOfSame:
                        ...
                        if exist_list.count(_name) > 0:
                            ...
                            if exist_list.count(_name) == 1:
                                repeat_list.append(_name + ',' + str(exist_list.count(_name) + 1) + '\n')
                            else:
                                repeat_list.remove(_name + ',' + str(exist_list.count(_name)) + '\n')
                                repeat_list.append(_name + ',' + str(exist_list.count(_name) + 1) + '\n')
                        text+=',' + "{:.1f}".format(preset_blank - 0.5 * ticks + float(count) * ticks)
                ...
                if OtoMaxOfSame == -1 or exist_list.count(_name) < OtoMaxOfSame:
                    ...
                    if exist_list.count(_name) > 0:
                        ...
                        if exist_list.count(_name) == 1:
                            ...
                            repeat_list.remove(_name + ',' + str(exist_list.count(_name)) + '\n')
                            repeat_list.append(_name + ',' + str(exist_list.count(_name) + 1) + '\n')
                    text+=',' + "{:.1f}".format(preset_blank - 0.1 * ticks + float(count) * ticks)
...
_oto_divide_vccv = ''.join(re.split(r'[,=]+', ini[13])[1]).strip('\n') == 'True'

my_worker = worker()
my_worker.read_presamp(_input_path)
my_worker.gen_CVVC(_reclist_output_path,_length,_use_planb, _include_CV_head,_include_VV,_use_underbar,_oto_output_path,_oto_max_of_same,_oto_preset_blank,_oto_bpm,_oto_divide_vccv)